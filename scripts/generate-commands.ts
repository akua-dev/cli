...
export function collectPublicCommands(
  ...
  if (!optionalSecurityRequirements(spec.security))
    return invalidSpec("OpenAPI root security must be an array");
...
function renderCommandRegistry(commands: readonly CommandDefinition[]): string {
  return (
    ...
    `import type { PublicOperationId } from "./public-operation-executor.gen";\n` +
    `import type { CommandDefinition } from "../runtime/registry";\n\n` +
    `export const commandRegistry: readonly CommandDefinition<PublicOperationId>[] = ${JSON.stringify(commands, null, 2)};\n`
  );
}
function isOpenApiOperation(value: unknown): value is OpenApiOperation {
  return (
    ...
    optionalSecurityRequirements(value.security) &&
...
function optionalSecurityRequirements(
  value: unknown,
): value is readonly OpenApiSecurityRequirement[] | undefined {
  return (
    value === undefined ||
    (Array.isArray(value) && value.every(isSecurityRequirement))
  );
}
function isSecurityRequirement(
  value: unknown,
): value is OpenApiSecurityRequirement {
  return (
    isRecord(value) &&
